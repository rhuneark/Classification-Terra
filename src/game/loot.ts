import type { Item, LootEvent, LogEntry } from './types.ts';
...
export function rollLootEvent(
    ...
    const ambushRoll = luckBonus
        ? Math.random() * 1.4 // luck reduces ambush chance
        : Math.random();

    const isAmbush = ambushRoll < location.ambushChance;

    if (!isAmbush) {
        const item = rollRandomItem(location.minRarity, location.maxRarity);
        return {
            type: 'loot',
    ...
    // Ambush: lose an inventory item or energy
...
export function eventToLogEntry(event: LootEvent): LogEntry {
    ...
    if (event.type === 'loot' && event.foundItem) {
        message = `[${event.locationName}] Found: ${event.foundItem.name}. ${event.flavorText}`;
